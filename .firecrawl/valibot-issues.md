...
`type` describes which function did the validation. If the schema function [`array`](https://valibot.dev/api/array/) detects that the input is not an array, `type` has the value `'array'`. If the [`minLength`](https://valibot.dev/api/minLength/) validation function detects that an array is too short, `type` has the value `'min_length'`.
...
`message` contains a human-understandable error message that can be fully customized as described in our [quick start](https://valibot.dev/guides/quick-start/#error-messages) and [internationalization](https://valibot.dev/guides/internationalization/) guide.
...
`requirement` can contain further validation information. For example, if the [`minLength`](https://valibot.dev/api/minLength/) validation function detects that a string is too short, `requirement` contains the minimum length that the string should have.
...
`issues` currently only occur when using [`union`](https://valibot.dev/api/union/) and contains all issues of the schemas of an union type.

#### Config

`lang` can be used as part of our [i18n feature](https://valibot.dev/guides/internationalization/) to define the required language. `abortEarly` and `abortPipeEarly` gives you an info that the validation was aborted prematurely. You can find more info about this in the [parse data](https://valibot.dev/guides/parse-data/#configuration) guide. These are all configurations that you can control yourself.
...
If you are only interested in the error messages of each issue to show them to your users, you can convert an array of issues to a flat object with [`flatten`](https://valibot.dev/api/flatten/). Below is an example.
...
However, with the help of [`flatten`](https://valibot.dev/api/flatten/) the issues were converted to the following object.
