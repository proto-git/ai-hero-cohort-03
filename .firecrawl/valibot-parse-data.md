...
If you want issues to be returned instead of thrown, you can use [`safeParse`](https://valibot.dev/api/safeParse/). The returned value then contains the `.success` property, which is `true` if the input is valid or `false` otherwise.
...
Another way to validate data that can be useful in individual cases is to use a type guard. You can use either a type predicate with the [`is`](https://valibot.dev/api/is/) method or an assertion function with the [`assert`](https://valibot.dev/api/assert/) method.
