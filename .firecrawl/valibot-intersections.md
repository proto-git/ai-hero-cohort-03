...
Technically, there is a big difference between [`intersect`](https://valibot.dev/api/intersect/) and object merging. [`intersect`](https://valibot.dev/api/intersect/) is a schema function that executes the passed schemas during validation. In contrast, object merging is done during initialization to create a new object schema.

As a result, object merging usually has much better performance than [`intersect`](https://valibot.dev/api/intersect/) when validating unknown data. Also, subsequent object properties overwrite the previous ones. This is not the case with [`intersect`](https://valibot.dev/api/intersect/), since the validation would fail if two properties with the same name are fundamentally different.
